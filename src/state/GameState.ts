...
function nextPendingAbortiveDrawAfterKan(
...
function applyDoubleRonPayments(
  players: readonly [PlayerData, PlayerData, PlayerData, PlayerData],
...
function clearTemporaryFuritenAndIppatsu(
  players: readonly [PlayerData, PlayerData, PlayerData, PlayerData],
): [PlayerData, PlayerData, PlayerData, PlayerData] {
...
export function processAiTurn(state: GameState): {
  ...
  if (state.phase === "claiming") {
    ...
    const claim = aiClaims.find((c) => {
      ...
      const simulated = simulateMeldClaim(state.players[c.player], c);
      return isMeldTanyaoAiming(c) || isMeldTenpaiMaking(simulated);
    });
    if (claim) {
