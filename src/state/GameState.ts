...
function handleExhaustiveDraw(state: GameState): GameState {
  ...
  const detail =
    tenpaiList.length > 0 ? `聴牌: ${tenpaiStr}  不聴: ${notenStr || "なし"}` : "全員不聴";
...
export function collectClaims(
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (
      ...
      isWinningHand(tilesToCounts([...allPlayerTiles(players[i]), discarded]))
    ) {
      options.push({
        type: "ron",
        player: i,
...
export function gameReducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    case "RON": {
      ...
      if (
        !isWinningHand(
          tilesToCounts([...allPlayerTiles(state.players[action.winner]), state.lastDiscard.tile]),
        )
      ) {
